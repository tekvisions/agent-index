...
def main() -> int:
    ...
    d.text((gx + 3 * (s + g) + 14, gy + 14), "THE AGENT INDEX", font=f_kick, fill=MUTED)

    # heavy title — "assembled." in violet
    d.text((66, 188), "The agent-building", font=f_h1, fill=INK)
    d.text((66, 296), "stack, ", font=f_h1, fill=INK)
    try:
        w_stack = d.textlength("stack, ", font=f_h1)
    except Exception:
        w_stack = 300
    d.text((66 + w_stack, 296), "assembled.", font=f_h1, fill=VERM)
